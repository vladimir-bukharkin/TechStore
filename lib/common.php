...
/*Функция вывода поплярных товаров*/
...
/*
 * Выполняет поиск в базе данных и загрузку товаров, добавленные казанным пользователем
 */
function db_car_find_by_user_id($dbh, $user_id)
{
    $query = 'SELECT * FROM car WHERE user_id=?';
    $result = array();
...
function db_product_incar_insert($dbh, $product)
{
    if(bd_is_current_product($dbh, $product['product_id'], $product['user_id']))
    {
        ...
        /*Проверка возможности добавления товара в корзину в зависимости от наличия товара на складе.
         * Если в корзине кол-во товаров меньше кол-ва товаров на складе, то можно прибавить, в другом случае,
...
/*Возвращает массив продуктов в корзине без описания, с добавлением полей цена товара (Nтовара*Kкол-во) и цена корзины */
function db_get_product_in_car_by_user($dbh)
{
    $user_car = db_car_find_by_user_id($dbh, $_SESSION['user_id']);
